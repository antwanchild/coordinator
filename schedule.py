...
def build_xlsx(people, room_data=None):
    ...
    for sheet_name in workbook.sheetnames:
        ...
        for row_index in range(ROWS):
            ...
            set_cell_value(
                ...
                sheet_row,
                3,
                "Off." if is_off_row else (safe_cell_value(person["name"]) if person else ""),
            )
                ...
                for room_index, room in enumerate(rooms):
                    ...
                    if not is_available:
                        ...
                        for slot_index in range(SLOTS):
                            worksheet.cell(row=sheet_row, column=start_col + slot_index).fill = (
                                gray_fill
                            )
    ...
    for person in sorted(people, key=lambda p: p["name"]):
        parts = [safe_cell_value(person["name"])] + [
            safe_cell_value(t) for r in person["ranges"] for t in (r["start"], r["end"])
        ]
