...
def render_preview(people, is_pm, room_data=None):
    ...
    draw_border(
        3, 1, colspan=1, rowspan=4, left="medium", right="medium", top="double", bottom="double"
    )
    ...
    for room_index, room in enumerate(rooms):
        ...
        draw.text(
            (x2l - room_label_w - 3, ty), "Room:", fill=hex_to_rgb("#000000"), font=font_bold14
        )
        ...
        draw.text(
            (x1l + (x2l - x1l - draw.textlength("Time:", font=font_bold)) // 2, ty),
            "Time:",
        ...
        for slot_index, slot_label in enumerate(SLOT_LABELS):
            ...
            draw_border(
                ...
                right=(
                    "medium"
                    if (is_last_room and is_last_slot)
                    else ("thin" if is_last_slot else None)
                ),
            )
    ...
    for row_index in range(ROWS):
        ...
        person = (
            None
            if is_off_row
            else (sorted_people[row_index - 1] if row_index - 1 < len(sorted_people) else None)
        )
        ...
        for room_index, room in enumerate(rooms):
            ...
            for slot_index in range(SLOTS):
                ...
                is_last_slot = slot_index == SLOTS - 1
                slot_color = (
                    SLOT_COLORS[slot_index]
                    if (is_off_row or not person or is_available)
    ...
    for footer_index, footer_label in enumerate(FOOTER_LABELS):
        ...
        for room_index, room in enumerate(rooms):
            ...
            if footer_index == 0:
                ...
                draw_border(
                    start_col, sheet_row, colspan=SLOTS, left="medium", top=top, right=right_border
                )
                ...
                checkbox_color = "#FF0000" if room["highlight_narrow_side"] else "#FFFFFF"
                fill_cell(start_col, sheet_row, checkbox_color)
                draw_border(
                    start_col, sheet_row, left="medium", right="thin", top="thin", bottom="thin"
                )
                fill_cell(start_col + 1, sheet_row, "#FFFFFF")
                if room["narrow_side_marker"]:
                    draw_text(start_col + 1, sheet_row, room["narrow_side_marker"], font_bold)
                draw_border(start_col + 1, sheet_row, left="thin", top="thin")
                fill_cell(start_col + 2, sheet_row, "#FFFFFF", colspan=SLOTS - 2)
                if room["thin_receivers_note"]:
                    draw_text(
                        start_col + 3,
                        sheet_row,
                        room["thin_receivers_note"],
                        font_medium,
                        "#FF0000",
                        "left",
                        8,
                ...
                if room["wheelchair_walker_note"]:
                    draw_text(
                        start_col + 1,
                        sheet_row,
                        room["wheelchair_walker_note"],
                        font_bold,
                        "#000000",
                        "left",
                        SLOTS - 1,
