...
Versions are bumped automatically from commit messages and the changelog is generated by [git-cliff](https://git-cliff.org/). The workflow looks at the commit range that reached `main`, so merge commits and mixed-case prefixes are handled more reliably. See [CONTRIBUTING.md](CONTRIBUTING.md) for the full convention. Quick reference:
